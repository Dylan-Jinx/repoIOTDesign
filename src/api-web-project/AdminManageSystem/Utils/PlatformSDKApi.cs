...
namespace AdminManageSystem.Utils
{
    public class PlatformSDKApi
    {
        ...
        /// </summary>
        /// <returns></returns>
        public CookHouseBillDTO GetAllCookHouseBill()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, SchoolCookHouseApiUrl.getallbills);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity responseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<CookHouseBillDTO>(responseEntity.Bodys);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public PlantInfoDTO GetAllPlantInfo()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, TraceableApiUrl.getallplantinfo);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity responseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<PlantInfoDTO>(responseEntity.Bodys);
        }
        /// <summary>
        /// 获取所有产品批次信息
        /// </summary>
        /// <returns></returns>
        public ProductNumInfoDTO GetAllProductNumInfo()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, TraceableApiUrl.getallproductnuminfo);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity httpResponseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<ProductNumInfoDTO>(httpResponseEntity.Bodys);
        }
        /// <summary>
        /// 获取所有种植基地信息
        /// </summary>
        /// <returns></returns>
        public PlantBaseInfoDTO GetAllPlantBaseInfo()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, TraceableApiUrl.getallplantbaseinfo);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity httpResponseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<PlantBaseInfoDTO>(httpResponseEntity.Bodys);
        }
        /// <summary>
        /// 获取所有仓库信息
        /// </summary>
        /// <returns></returns>
        public WareHouseInfoDTO GetAllWareHouseInfo()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, TraceableApiUrl.getallwarehouseinfo);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity httpResponseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<WareHouseInfoDTO>(httpResponseEntity.Bodys);
        }
        /// <summary>
        /// 获取所有水质检测信息
        /// </summary>
        /// <returns></returns>
        public WaterQualityDTO GetAllWaterQualityInfo()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, TraceableApiUrl.getallwaterqualityinfo);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity httpResponseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<WaterQualityDTO>(httpResponseEntity.Bodys);
        }
        /// <summary>
        /// 获取所有运输环境信息
        /// </summary>
        /// <returns></returns>
        public TransEnvironmentDTO GetAllTransferEnvironInfo()
        {
            string apipath = string.Format("{0}{1}", m_currenthost, TraceableApiUrl.getalltransferenvironinfo);
            HttpRequestEntity httpRequestEntity = new HttpRequestEntity()
            {
                Method = HttpMethod.GET,
            };
            HttpResponseEntity httpResponseEntity = HttpHelper.Get(apipath, httpRequestEntity);
            return JsonHelper.Deserialize<TransEnvironmentDTO>(httpResponseEntity.Bodys);
        }
        #endregion
    }
}
